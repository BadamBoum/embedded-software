...
byte valeur_octet[8]; // contient la valeur découpée en octet pour la réception
...
void loop()
{
  ...
  valeur = (valeur_octet[0] << 8) + valeur_octet[1]; // transforme en int
  Serial.print(valeur, DEC);
  Serial.print(" ");
  valeur = (valeur_octet[2] << 8) + valeur_octet[3]; // transforme en int
  Serial.print(valeur, DEC);
  Serial.print(" ");
  valeur = (valeur_octet[4] << 8) + valeur_octet[5]; // transforme en int
  Serial.print(valeur, DEC);
  Serial.print(" ");
  valeur = (valeur_octet[6] << 8) + valeur_octet[7]; // transforme en int
